...
def welcome():
      ...
      if dbconn.con:
          sql="SELECT * FROM loginregistration"
...
def signup():
       if request.method == 'POST':
         ...
         sql = "INSERT INTO  loginregistration (username,email,password,rollnumber) VALUES ('{0}','{1}','{2}','{3}')"
...
def signin():
       if request.method == 'POST':
            email = request.form.get('email')
            pwd = request.form.get('pwd')
            sql = "SELECT * FROM  loginregistration WHERE EMAIL='{0}' AND PASSWORD='{1}'"
...
def update(id):
            if request.method =='POST':
               ...
               pwd = request.form.get('pwd')
               roll = request.form.get('roll')
               sql="UPDATE  loginregistration SET username='{0}',email='{1}',password='{2}',rollnumber='{3}' WHERE id='{4}'"
            ...
            sql="SELECT * FROM  loginregistration WHERE id='{0}'"
...
def delete(id):
         sql="DELETE FROM  loginregistration WHERE id='{0}'"
         smt=ibm_db.exec_immediate(dbconn.con,sql.format(id))
         return redirect("/welcome")
