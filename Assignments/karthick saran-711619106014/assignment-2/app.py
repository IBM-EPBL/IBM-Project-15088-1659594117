...
def signup():
       if request.method == 'POST':
         ...
         sql = "INSERT INTO authentication (username,email,password,rollnumber) VALUES ('{0}','{1}','{2}','{3}')"
...
def update(id):
            if request.method =='POST':
               ...
               pwd = request.form.get('pwd')
               roll = request.form.get('roll')
               sql="UPDATE authentication SET username='{0}',email='{1}',password='{2}',rollnumber='{3}' WHERE id='{4}'"
